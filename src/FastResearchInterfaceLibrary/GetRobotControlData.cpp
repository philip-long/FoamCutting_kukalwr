...
// GetEstimatedExternalCartForcesAndTorques()
//
void FastResearchInterface::GetMeasuredCartForcesAndTorques(float *MeasuredCartForcesAndTorques)
{
	unsigned int		i	=	0;
	...
	for (i = 0; i < FRI_CART_VEC; i++)
	{
		MeasuredCartForcesAndTorques[i]	=	this->Force_sensors.FTData[i];
	}
	pthread_mutex_unlock(&(this->MutexForControlData));
